...
<?php
$order_button_text            = apply_filters( 'learn_press_order_button_text', __( 'Place order', 'educatito' ) );
$order_button_text_processing = apply_filters( 'learn_press_order_button_text_processing', __( 'Processing', 'educatito' ) );
$show_button                  = true;
...
<div id="learn-press-payment" class="learn-press-checkout-payment">
	...
	<?php if ( LP()->cart->needs_payment() ) { ?>
		...
		<?php if ( ! $count_gateways ) { ?>
			...
			<?php if ( $message = apply_filters( 'learn_press_no_available_payment_methods_message', __( 'No payment method is available.', 'educatito' ) ) ) { ?>
				<?php learn_press_display_message( $message, 'error' ); ?>
			<?php } ?>

		<?php } else { ?>

            <h4><?php esc_html__( 'Payment Method', 'educatito' ); ?></h4>
        ...
        <div id="checkout-order-action" class="place-order-action">
			...
			<?php if ( ! is_user_logged_in() ) { ?>
                <button type="button" class="lp-button lp-button-guest-checkout"
                        id="learn-press-button-guest-checkout-back"><?php esc_html__( 'Back', 'educatito' ); ?></label></button>
			<?php } ?>
