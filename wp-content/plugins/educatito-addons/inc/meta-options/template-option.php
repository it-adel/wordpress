 ...
 * @author JRB Themes
 */
global $educatito_meta_options;

$educatito_meta_options = new MetaOption();

function educatito_options($params = array()) {
    global $pagenow, $educatito_meta_options;
    ...
    /* Find Type */
    if (is_admin() && !empty($params['id']) && isset($educatito_meta_options)) {
        /* Taxonomys */
        if ($pagenow == 'edit-tags.php') {
            ...
            $educatito_meta_options->taxonomy($params);
        }
        /* Post or Page */ elseif ($pagenow == 'post-new.php' || $pagenow == 'post.php') {
            global $post;
            /* Get jrb meta data opject */
            $educatito_meta = json_decode(get_post_meta($post->ID, '_jrb_meta_data', true));

            if (!empty($educatito_meta)) {
                foreach ($educatito_meta as $key => $meta) {
                    $educatito_meta->$key = rawurldecode($meta);
                }
            }

            // Render params id
            $params['id'] = "_jrb_" . $params['id'];

            $educatito_meta_array = objectToArray($educatito_meta);

            // Get value
            if (!empty($educatito_meta_array[$params['id']])) {
                $params['value'] = $educatito_meta_array[$params['id']];
            } else {
                $params['value'] = null;
            }

            $educatito_meta_options->metabox($params);
        }
    } else {
        esc_html_e('Error', 'educatito');
    }
...
class MetaOption {
    ...
    function __construct() {
        ...
        add_action('save_post', array($this, 'educatito_save_meta_boxes'));
    }
    ...
    function Scripts() {
        ...
        wp_register_style('font-awesome', EDUCATITO_PLUGIN_URL . '/inc/meta-options/css/font-awesome.min.css', array(), '4.1.0');
        wp_register_style('font-ionicons', EDUCATITO_PLUGIN_URL . '/inc/meta-options/css/ionicons.min.css', array(), '1.5.2');
        wp_register_style('jquery-datetimepicker', EDUCATITO_PLUGIN_URL . '/inc/meta-options/css/jquery.datetimepicker.css');
        wp_register_style('jquery-minicolors', EDUCATITO_PLUGIN_URL . '/inc/meta-options/css/jquery.minicolors.css');
        wp_register_style('core-options', EDUCATITO_PLUGIN_URL . '/inc/meta-options/css/meta-options.css');
        wp_register_style('jquery.nouislider', EDUCATITO_PLUGIN_URL . '/inc/meta-options/css/jquery.nouislider.css');
        wp_register_style('jquery.nouislider.pips', EDUCATITO_PLUGIN_URL . '/inc/meta-options/css/jquery.nouislider.pips.css');

        wp_register_script('jquery-datetimepicker', EDUCATITO_PLUGIN_URL . '/inc/meta-options/js/jquery.datetimepicker.js');
        wp_register_script('jquery-minicolors', EDUCATITO_PLUGIN_URL . '/inc/meta-options/js/jquery.minicolors.js');
        wp_register_script('media-selector', EDUCATITO_PLUGIN_URL . '/inc/meta-options/js/media.selector.js');
        wp_register_script('jquery.nouislider', EDUCATITO_PLUGIN_URL . '/inc/meta-options/js/jquery.nouislider.all.js');
        wp_register_script('icons-class', EDUCATITO_PLUGIN_URL . '/inc/meta-options/js/icons.class.js');
        wp_register_script('upload', EDUCATITO_PLUGIN_URL . '/inc/meta-options/js/upload.js');
        wp_register_script('core-options', EDUCATITO_PLUGIN_URL . '/inc/meta-options/js/meta-options.js');
    }
    ...
    private function text($params) {
        ...
        ?>
            <input type="text"
                   ...
                   value="<?php if (isset($params['value'])) {
            echo esc_attr($params['value']);
        } ?>"
                   placeholder="<?php if (isset($params['placeholder'])) {
    ...
    private function textarea($params) {
        ...
        <div class="area-field csfield">
            <textarea<?php if (isset($params['rows'])) {
        ...
        <?php foreach ($params['options'] as $key => $option): ?>
                    <option value="<?php echo esc_attr($key); ?>" <?php if (isset($params['value']) && ($params['value'] == $key)) {
                echo "selected";
    ...
    private function imegesselect($params) {
        ...
        <?php foreach ($params['options'] as $key => $image): ?>
                    <li data-value="<?php echo esc_attr($key); ?>" class="<?php if ($params['value'] == $key) {
                echo 'active';
            } ?>"><img alt="" src="<?php echo esc_url($image) ?>"></li>
                <?php endforeach; ?>
            </ul>
    ...
    private function multiple($params) {
        ...
        <?php foreach ($params['options'] as $key => $option): ?>
                    <option value="<?php echo esc_attr($key); ?>" <?php if (in_array($key, $selected)) {
        ...
        <div class="slider-field xfield">
            <div class="slider-main">
                ...
                <div class="slider-max">
                    ...
                    <span class="unit"><?php if (isset($params['unit'])) {
            echo esc_attr($params['unit']);
        } ?></span>
                </div>
            </div>
            <input type="hidden"
    ...
    private function xswitch($params) {
        ...
        ob_start();
        ?>
        <div class="switch-field csfield<?php if ($params['value'] == $options['on']) {
            echo ' on';
        } else {
            echo ' off';
            ...
            if (count($attachment_image) > 0):
                ?>
                        <li class="items" data-id="<?php echo esc_attr($value); ?>" style="background-image:url(<?php echo esc_url($attachment_image[0]); ?>);background-size:cover;">
                            <i class="edit dashicons dashicons-plus-alt" title="<?php esc_html_e('Replace Image', 'educatito'); ?>"></i>
                            <i class="remove dashicons dashicons-dismiss" title="<?php esc_html_e('Remove Image', 'educatito'); ?>"></i>
                        </li>
                <?php endif;
            endforeach; ?>
            <?php if ($params['field'] != 'single'): ?>
                    <li class="items" data-id=""><i class="add dashicons dashicons-plus-alt" title="<?php esc_html_e('Add Image', 'educatito'); ?>"></i></li>
    ...
    private function file($params) {
        ...
        <div class="file-field csfield">
            <input name="<?php echo esc_attr($params['id']); ?>" class="upload_field" id="<?php echo esc_attr($params['id']); ?>" type="text" value="<?php if (!empty($params['value'])) {
            echo esc_attr($params['value']);
    ...
    private function datetime($params) {
        ...
        ob_start();
        ?>
        <div class="date-field" data-type="<?php echo esc_attr($params['field']); ?>" data-format="<?php if (!empty($params['format'])) {
            echo esc_attr($params['format']);
        } else {
            echo 'm/d/Y';
        } ?>">
        <?php
        ...
        ?>
            <input type="text" name="<?php echo esc_attr($params['id']); ?>" id="<?php echo esc_attr($params['id']); ?>" class="text-indent xvalue" value="<?php if (!empty($params['value'])) {
            echo esc_attr($params['value']);
        } ?>" placeholder="<?php echo esc_attr($params['placeholder']); ?>"/>
        </div>
        <?php
    ...
    public function metabox($params) {
        ...
        <?php endif; ?>
            <div class="field<?php if (isset($params['class'])) {
    ...
    public function taxonomy($params) {
        ...
        <tr class="form-field">
            <th scope="row" valign="top">
                <label class="field-title" for="<?php echo esc_attr($params['id']); ?>"><?php if (isset($params['label'])) {
            echo esc_attr($params['label']);
        } ?></label>
            </th>
            <td<?php if (isset($params['class'])) {
            echo ' class="' . $params['class'] . '"';
        } ?>>
        <?php echo '' . $this->renderType($params); ?>
                <br />
                <span class="description"><?php if (isset($params['desc'])) {
            echo esc_attr($params['desc']);
        } ?></span>
            </td>
        </tr>
        <?php
     ...
     */
    public function educatito_save_meta_boxes($post_id) {
        /* doing save. */
        ...
        foreach ($_POST as $key => $value) {
            if (strstr($key, '_jrb_')) {
                $educatito_meta[$key] = rawurlencode($value);
            }
        }
        /* update _jrb_meta_data. */
        if (!empty($educatito_meta)) {
            update_post_meta($post_id, '_jrb_meta_data', json_encode($educatito_meta));
        }
    }
