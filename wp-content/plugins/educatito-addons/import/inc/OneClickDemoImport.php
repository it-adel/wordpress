...
class OneClickDemoImport {
  ...
  private $plugin_page_setup = array();
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'educatito' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'educatito' ),
						'dialog_no'             => esc_html__( 'Cancel', 'educatito' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'educatito' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'educatito' ),
					),
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'educatito' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'educatito' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'educatito' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'educatito' ) );
			}
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= sprintf(
				__( '%1$s%5$sThat\'s it, all done!%6$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. Contact %3$sjrbthemes@gmail.com%4$s for help.%7$s', 'educatito' ),
				'<div class="notice  notice-success"><p>',
				'<br>',
				'<a href="mailto:jrbthemes@gmail.com">',
				'</a>',
                                '<strong>',
				'</strong>',
				'</p></div>'
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s% .Contact %7$sjrbthemes@gmail.com%8$s for help9$s', 'educatito' ),
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'educatito', false, plugin_basename( dirname( dirname( __FILE__ ) ) ) . '/languages' );
	}
